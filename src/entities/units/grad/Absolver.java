  ...
  private void A1(){
    if(aiming){
      if(dir == 1 && handler.getMouseManager().isLeftPressed() && !hasMouse()){
        rangedCheck((int)(x), (int)(y - 256), 64, 256, 30, 0, 0);
        click();
        aiming = false;
        if(rangedCheck((int)(x), (int)(y - 256), 64, 64, 0, 0, 0) > 0){
          y = y - 320;
        }else{
          y = y-256;
        }
        ...
        rangedCheck((int)(x), (int)(y), 64, 320, 30, 0, 0);
        click();
        aiming = false;
        if(rangedCheck((int)(x), (int)(y + 256), 64, 64, 0, 0, 0) > 0){
          y = y + 320;
    ...
    if(aiming){
      if(dir == 1 && handler.getMouseManager().isLeftPressed()&& !hasMouse()){
        if(proc(16)){
          ...
          rangedCheck((int)(x), (int)(y-64), 64, 64, 40, 0, 0);
        }else{
          rangedCheck((int)(x), (int)(y-64), 64, 64, 80, 0, 0);
        }
        click();
        aiming = false;
      }else if(dir == 3 && handler.getMouseManager().isLeftPressed()&& !hasMouse()){
